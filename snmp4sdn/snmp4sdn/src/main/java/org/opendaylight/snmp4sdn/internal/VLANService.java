...
import org.opendaylight.snmp4sdn.internal.NexusCliHandler;
...
public class VLANService implements /*IPluginInVLANService,//custom ad-sal*/ VlanService/*md-sal*/, IVLANService/*not to remove this interface, otherwise code vary a lot */, IKarafVLANService, CommandProvider{
    ...
    public Future<RpcResult<java.lang.Void>> printVlanTable(final PrintVlanTableInput input){
        Long nodeId = input.getNodeID();
        Node node = createSNMPNode(nodeId.longValue());
        System.out.println("in md-sal printvlanTable");
    ...
    public void printVLANTable(String sw_mac){
        ...
        System.out.println("VLANService.printVLANTable() is called by Karaf");
   ...
   private boolean s4sAddVLAN_execute(String sw_mac, String vlanID, String vlanName){
    ...
    @Override//karaf
    public void setAllVlans(String dbPath){
      //  cmethUtil.setAllVlans(filepath);
     // vlanstable.clear();
           System.out.println("Supporting setAllVlans command");
           FileReader FileStream=null;
           BufferedReader BufferedStream=null;
           try{
        System.out.println("openning file ");
        //System.out.println(dbPath);//  "/home/christine/snmp4sdn/snmp4sdn/src/test/switch_login_db.csv"
        //ToDo: this part somehow should be added here  
         FileStream=new FileReader(dbPath); //"/home/christine/snmp4sdn/snmp4sdn/src/test/switch_login_db.csv"
       ...
       String line = null;
        ...
        while(true){
            ...
            String sw_mac, vlan_id, vlan_name, port_list;

            String str = new String(line).trim();
            sw_mac = str.substring(0, str.indexOf(",")).trim();
            //if(sw_mac.startsWith(","))logger.warn("mac field is empty");

            str = str.substring(str.indexOf(",") + 1).trim();
            vlan_id = str.substring(0, str.indexOf(",")).trim();
            //if(vlan_id.startsWith(","))logger.warn("vlan field is empty");

            str = str.substring(str.indexOf(",") + 1).trim();
            vlan_name = str.substring(0, str.indexOf(",")).trim();
            //if(vlan_name.startsWith(","))logger.warn("vlan name field is empty");

            port_list = str.substring(str.indexOf(",") + 1).trim();
            //port_list = str.substring(0, str.indexOf(",")).trim();
            //if(port_list.startsWith(","))logger.warn("portlist starts with an empty field");
            //                        //logger.trace("ip: " + ip);
            
            //ToDo
            //set seperate vlans
            
             addVLANSetPorts(sw_mac, vlan_id, vlan_name, port_list); 
 }
        }catch(Exception e){
            logger.error("CmethUtil.setAllVLAN() err: {}", e);
        }
    }
    ...
    private boolean s4sSetVLANPorts_execute(String sw_mac, String vID, String portList){
        ...
        try{
            ...
            for(int i = 0; i < ports.length; i++)
                nodeConns.add(new NodeConnector("SNMP", Integer.parseInt(ports[i]), node));
        }catch(Exception e){
        ...
        if(status.isSuccess())
            ...
            return false;
            }//end of if nexus ==false
       else {
          System.out.println("s4sSetVLANPorts_execute"); 
          Status status = new  NexusCliHandler(cmethUtil).setVLANPorts(node, vlanID, portList );
          if(status.isSuccess())
            return true;
        else
            return false;
    ...
    private void s4sPrintVLANTable_execute(String sw_mac){
        ...
        System.out.println("in s4sprintvlanExec");
    ...
    return new Status(StatusCode.NOTACCEPTABLE, "wrong call for a NEXUS switch");
    }
