...
public class CmethUtil implements CommandProvider{
    ...
    public void readDB(String dbPath){
        ...
        while(true){
            ...
            String mac, ip, snmp_community, cli_username, cli_password, switch_type;
            ...
            str = str.substring(str.indexOf(",") + 1).trim();
            switch_type = new String(str);
            if(cli_password.startsWith(","))logger.warn("cli_password field is empty");
            //logger.trace("cli_password: " + cli_password);

            addEntry(HexString.toLong(mac), ip, snmp_community, cli_username, cli_password, switch_type);
        }
        }catch(Exception e){
    ...
    public void addEntry(Long mac, String ip, String snmp_community, String cli_username, String cli_password, String switch_type){
        Vector entryVec = new Vector();
        entryVec.add(ip);
        entryVec.add(snmp_community);
        entryVec.add(cli_username);
        entryVec.add(cli_password);
        entryVec.add(switch_type);
    ...
    public String getCliPassword(Long macAddr){
     ...
     public String getSwitchType(Long macAddr){
        Vector entryVec = table.get(macAddr);
        if(entryVec == null)
            return null;
        String swType = (String)(entryVec.get(4));
        return swType;
    }
    ...
    public void _s4sReadDB(CommandInterpreter ci){
        System.out.print("MY FIRST LITTLE TEST\n");
     ...
     public void printDB(){
         ...
         for (ConcurrentMap.Entry<Long, Vector> entry: table.entrySet()) {
            ...
            System.out.println(getSwitchType(mac));
        }
     }
}
